...
import { localAiDriver, type AiPayload } from "@/server/ai-driver";
...
// Development-only proxy for the explicit cloudflare driver; Wrangler owns OAuth renewal and remote binding access.
let localAi: Promise<AiBinding> | undefined;
async function getLocalAi(): Promise<AiBinding | undefined> {
  if (process.env.NODE_ENV !== "development" || process.env.CLOUDFLARE_AI_DRIVER !== "cloudflare") return undefined;
...
export async function generateAiResponse(payload: AiPayload): Promise<AiResponse> {
  if (process.env.NODE_ENV === "development" && process.env.CLOUDFLARE_AI_DRIVER !== "cloudflare") {
    return localAiDriver.generate(payload);
  }
  ...
  if (raw === undefined) return localAiDriver.generate(payload);
  ...
  if (!alignsWithRequestedQuestionDirection(payload, normalized)) {
    const fallback = await localAiDriver.generate(payload);
